...
<template>
  ...
  >
    ...
    <div id="submitMessage" class="m-0 w-full h-fit p-0">
      <div
        class="container bg-white shadow m-auto w-4/5 my-4 max-w-md rounded-lg"
      ...
      <div
        class="container bg-white shadow w-4/5 my-4 max-w-md rounded-lg fixed"
      ...
      <div class="border-b border-gray-900/10 pb-12">
        ...
        <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
          <div class="sm:col-span-3">
            <label for="primer_nombre" class="block text-sm font-medium leading-6 text-gray-900"
              >Primer nombre</label
            >
            <div class="mt-2">
              <Field
                v-model="empleado.primer_nombre"
                name="primer_nombre"
                rules="required"
                id="primer_nombre"
                type="text"
          ...
          <div class="sm:col-span-3">
            <label for="segundo_nombre" class="block text-sm font-medium leading-6 text-gray-900"
              >Segundo nombre</label
            >
            <div class="mt-2">
              <Field
                v-model="empleado.segundo_nombre"
                name="segundo_nombre"
                id="segundo_nombre"
                type="text"
          ...
          <div class="sm:col-span-3">
            <label for="primer_apellido" class="block text-sm font-medium leading-6 text-gray-900"
          ...
          <div class="sm:col-span-3">
            <label for="segundo_apellido" class="block text-sm font-medium leading-6 text-gray-900"
          ...
          <div class="sm:col-span-3">
            <label for="fecha_nacimiento" class="block text-sm font-medium leading-6 text-gray-900"
          ...
          <div class="sm:col-span-3">
            <label for="estado_familiar" class="block text-sm font-medium leading-6 text-gray-900"
              >Estado familiar</label
            >
            <div class="mt-2">
              <Field
                v-model="empleado.id_estado_familiar"
                as="select"
              ...
              >
                ...
                <option
                  v-for="est in estado_familiar"
                  :key="est.id_estado_familiar"
                  :value="est.id_estado_familiar"
                >
                  {{ est.nombre_estado_familiar }}
                </option>
              </Field>
            </div>
          </div>
          ...
          <div class="sm:col-span-3">
            <label for="profesion" class="block text-sm font-medium leading-6 text-gray-900"
            ...
            <div class="mt-2">
              <Field
                ...
                rules="required"
                v-model="empleado.profesion_oficio"
                id="profesion"
                type="text"
                placeholder="Ingresa profesion u oficio"
                class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6"
              />
          ...
          <div class="sm:col-span-3">
            <label for="domicilio" class="block text-sm font-medium leading-6 text-gray-900"
              >Domicilio</label
            >
            <div class="mt-2">
              <Field
          ...
          <div class="sm:col-span-3">
            <label for="residencia" class="block text-sm font-medium leading-6 text-gray-900"
              >Residencia</label
            >
            <div class="mt-2">
              <Field
          ...
          <div class="sm:col-span-3">
            <label for="nacionalidad" class="block text-sm font-medium leading-6 text-gray-900"
              >Nacionalidad</label
            >
            <div class="mt-2">
              <Field
                v-model="empleado.id_nacionalidad"
                as="select"
                name="nacionalidad"
                id="nacionalidad"
                class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:max-w-xs sm:text-sm sm:leading-6"
              >
                ...
                <option
                  ...
                  :key="nac.id_nacionalidad"
                  :value="nac.id_nacionalidad"
                >
          ...
          <div class="sm:col-span-3">
            <label for="dui_empleado" class="block text-sm font-medium leading-6 text-gray-900"
              >DUI</label
            >
            <div class="mt-2">
              <Field
    ...
    <div class="container bg-white shadow m-auto w-4/5 my-4">
      <div class="sm:mx-auto sm:w-full sm:max-w-sm">
        <h2 class="pt-8 text-center text-2xl font-bold leading-9 tracking-tight text-gray-900">
          Datos del usuario
        </h2>
      </div>
      <div class="w-full text-center flex justify-center" v-if="!createForm">
        <router-link
          :to="{ name: 'gestion_usuarios' }"
          class="m-2 bg-purple-800 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline"
          >Editar usuarios</router-link
        >
      </div>
      <div class="w-full" v-if="createForm">
        <div class="flex min-h-full flex-col justify-center px-6 pt-4 pb-8 lg:px-8">
          <div class="mt-auto sm:mx-auto sm:w-full sm:max-w-sm">
            <div class="mb-4">
              <label for="usuario" class="block text-sm font-medium leading-6 text-gray-900"
                >Usuario</label
              >
              <div class="mt-2">
                <Field
    ...
    <div class="flex items-center justify-center">
      <router-link
        to="/recursos_humanos/listar_empleados"
...
export default {
  ...
  methods: {
    getCargos() {
      axios.get(api_url + '/cargos').then((response) => (this.cargos = response.data))
    },

    getNacionalidades() {
      axios
        .get(api_url + '/nacionalidades')
        .then((response) => (this.nacionalidades = response.data))
    },
    getSexos() {
      axios.get(api_url + '/sexos').then((response) => (this.sexos = response.data))
    },
    getEstadoFamiliar() {
      axios
        .get(api_url + '/estado_familiar')
        .then((response) => (this.estado_familiar = response.data))
    },
    getEmpleado() {
      axios
        .get(api_url + '/empleado/' + this.id)
        .then(
          (response) => (
    ...
    saveEmpleado(values) {
      ...
      if (this.createForm != null) {
        ...
        //console.log(values,null,2);
        axios
          .put(api_url + '/empleado_update/' + this.id, params)
          .then((response) => {
            response.data.message.forEach((mensaje) => {
              showMessages(response.data.status, mensaje)
            })
