        ...
        <div class="max-w-2xl w-xl p-6 bg-white rounded-md shadow-xl">
          ...
          <div class="flex justify-center">
            <button
              ...
              v-bind:disabled="isfechaPosterior"
              v-bind:class="{
                'bg-slate-400 cursor-not-allowed': isfechaPosterior,
                'bg-blue-500 hover:bg-blue-700 border-2 border-blue-500 hover:border-blue-700 text-white ':
                  !isfechaPosterior
              }"
              class="px-4 py-2 rounded-lg mx-1"
            >
              Generar
            </button>
            <button
              @click="cerrarModal"
              class="bg-transparent hover:bg-slate-300 border-2 border-slate-300 text-slate-500 px-4 py-2 rounded-lg mx-1"
            >
              Cancelar
            </button>
          </div>
  ...
  methods: {
    ...
    verificarFecha() {
      ...
      if (fechaEspecificada.getUTCDate() == 1) {
        ...
        let ultimoDiaMes = new Date(
          fechaEspecificada.getUTCFullYear(),
          fechaEspecificada.getUTCMonth() + 1,
          0
        ).getDate()
    ...
    setPeriodoActual() {
      ...
      if (fechaActual.getUTCDate() <= 15) {
        this.dia = 16
        mes = Number(
          fechaActual.getUTCMonth(fechaActual.setUTCMonth(fechaActual.getUTCMonth() - 1)) + 1
        )
      } else {
        this.dia = '01'
      }
    ...
    generarPlanilla() {
      ...
      axios.post(api_url + '/planilla', params).then((response) => {
        ;(this.mensaje = response.data.mensaje),
          this.showMessages(response.data.status, this.mensaje)
          this.$emit('after_generar_planilla')
        this.$router.push('/recursos_humanos/historial_planillas')
        //this.$router.go(0);
      })
      this.cerrarModal()
    },
