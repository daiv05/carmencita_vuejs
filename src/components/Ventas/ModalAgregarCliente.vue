  ...
  <Transition name="modal">
    <div v-if="show" class="modal-mask bg-white">
      <div class="modal-container">
        <p class="text-xl font-bold mb-6 text-center">Registrar nuevo cliente</p>
        <div class="mb-4">
          <div class="grid grid-cols-2">
            ...
            <div class="col-span-1 ml-4">
              ...
              <input
                v-model="cliente.distintivo_cliente"
                type="text"
                id="identificador"
                class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6"
          ...
          <div class="grid grid-cols-2">
            ...
            <div class="col-span-1 mt-4">
              <label class="block text-gray-700 text-sm font-bold mb-2" for="dui"> DUI </label>
              <input
                @input="formatear_dui(cliente.dui_cliente)"
                v-model="cliente.dui_cliente"
                type="text"
                id="dui"
                class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6"
                placeholder="DUI"
            ...
            <div class="col-span-1 mt-4 ml-4">
              ...
              <input
                @input="formatear_nit(cliente.nit_cliente)"
                v-model="cliente.nit_cliente"
                type="text"
                id="nit"
                class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6"
                placeholder="NIT"
          ...
          <div class="grid grid-cols-2">
            <!--NRC-->
            <div class="col-span-1 mt-4">
              <label class="block text-gray-700 text-sm font-bold mb-2" for="nrc"> NRC </label>
              <input
                @input="formatear_nrc(cliente.nrc_cliente)"
                v-model="cliente.nrc_cliente"
                type="text"
                id="nrc"
                class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6"
                placeholder="NRC"
            ...
            <div class="col-span-1 mt-4 ml-4">
              ...
              >
                ...
                <option
                  v-for="departamento in departamentos_listado"
          ...
          <div class="grid grid-cols-2">
            ...
            <div class="col-span-1 mt-4">
              ...
              >
                <option disabled selected>Seleccione...</option>
                <option
                  v-for="municipio in municipios_listado"
            ...
            <div class="col-span-1 mt-4 ml-4">
              ...
              <input
                @input="limite_direccion(cliente.direccion_cliente)"
                v-model="cliente.direccion_cliente"
                type="text"
                id="direccion"
                class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6"
...
export default {
  ...
  methods: {
    ...
    actualizar_municipios() {
      axios
        .get(api_url + '/get_municipios?departamento=' + this.departamento_select)
        .then((res) => {
    ...
    registrar_nuevo_cliente() {
      if (
        ...
        this.cliente.distintivo_cliente == '' ||
        this.cliente.nrc_cliente == '' ||
        this.cliente.direccion_cliente == '' ||
        this.cliente.id_municipio == 'Seleccione...'
      ) {
      ...
      axios
        .post(api_url + '/clientes', this.cliente)
        .then((res) => {
          this.watch_toast('success', 'Cliente registrado exitosamente')
          this.$emit('close')
        })
        .catch((err) => {
          console.log(err)
          err.response.data.distintivo_exist
            ? this.watch_toast('error', 'Ya existe un cliente con ese identificador') &&
              document.getElementById('identificador').focus()
            : this.watch_toast('error', 'Ocurrió un error al actualizar, vuelva a intentar')
        })
    },
    formatear_dui(input) {
      ...
      if (nitValue.length > 4) {
        if (nitValue.length > 10) {
          if (nitValue.length > 13) {
            formattedNIT =
              nitValue.substring(0, 4) +
              '-' +
              nitValue.substring(4, 10) +
              '-' +
              nitValue.substring(10, 13) +
              '-' +
              nitValue.substring(13)
          } else {
            formattedNIT =
              nitValue.substring(0, 4) +
              '-' +
