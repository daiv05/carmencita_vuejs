    ...
    <div v-if="show" class="modal-mask bg-white">
      <div class="modal-container">
        ...
        <div class="mb-4 flex flex-row items-center justify-center">
          <p v-if="cliente.estado_cliente == 1" for="fecha_pedido text-center">
            ¿Está seguro que desea desactivar a este cliente?
          </p>
          <p v-else for="fecha_pedido text-center">
            ¿Está seguro que desea activar a este cliente?
          </p>
        </div>
        <div class="mb-4 flex flex-row items-center justify-center">
          <p v-if="cliente.estado_cliente == 1" class="text-center font-bold text-blue-950">
            La selección del cliente dejará de estar disponible en el registro de ventas
          </p>
          <p v-else class="text-center font-bold text-blue-950">
            El cliente estará disponible en el registro de ventas
          </p>
        </div>
        <div>
          <div class="text-center">
            <button
              @click="desactivar_cliente()"
...
export default {
  ...
  methods: {
    desactivar_cliente() {
      axios
        .post(api_url + '/clientes/cambiar_estado/' + this.cliente.id_cliente)
        .then((response) => {
