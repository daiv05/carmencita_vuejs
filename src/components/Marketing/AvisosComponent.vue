    ...
    <div v-if="avisos.length > 0">
      <transition name="fade" mode="out-in" class="bg-primary">
        <div :key="currentAviso.id" class="alert">
          ...
          <p class="mt-2 w-100 text-lg font-semibold text-left text-white">
            {{ currentAviso.titulo_aviso }}
          </p>
          <p class="mt-2 w-100 text-sm font-semibold text-left text-white">
            {{ currentAviso.cuerpo_aviso }}
          </p>
        </div>
      </transition>
    </div>
