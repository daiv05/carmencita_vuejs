...
export default {
  ...
  methods: {
      ...
      axios
        ...
        .then((response) => {
          ...
          try {
            ...
            toast.error(
              'El producto no tiene lista de precios extra. Registre precios extra primero.'
            )
          }
    ...
    enviarFormulario(event, values) {
      ...
      axios
        .post('/api/gestion_existencias/' + this.lote.id_lote + configuracionPut, dataForm)
        .then((response) => {
          dataForm.producto = this.lote.producto
          this.$emit('guardarLoteModificado', {
            dataForm: dataForm,
            mensaje: `Se edito el lote ${this.lote.id_lote} con éxito`
          })
