<template>
  <button
    v-if="credito.pendiente != 0"
    type="button"
    @click="isOpen = true"
    class="bg-transparent hover:bg-blue-500 text-blue-700 font-semibold hover:text-white py-2 px-4 border border-blue-500 hover:border-transparent rounded-full m-1"
  >
    Abonar
  </button>
  ...
  <span
    v-if="credito.pendiente == 0"
    class="bg-transparent hover:bg-blue-300 text-blue-500 font-semibold hover:text-white py-2 px-4 border border-blue-500 hover:border-transparent rounded-full m-1 cursor-not-allowed"
    >Abonar</span
  >
    ...
    <div class="w-9/12 lg:w-2/4 p-6 bg-white rounded-md shadow-xl">
      ...
      <div class="flex items-center m-2 justify-between">
        <h3 class="text-2xl text-center w-full">
          Credito {{ credito.id }} - {{ credito.proveedor.nombre_proveedor }}
        </h3>
      </div>
      <!--DETALLE DE CREDITO-->
      <div class="mt-4 text-center">
        ...
        <div class="">
          <p class="mb-4 text-md text-blue-700 font-bold text-center">Nuevo Abono:</p>
        </div>
        <div class="grid grid-cols-2 mb-4">
          <div class="flex justify-center items-center">
            <label class="mx-2 block text-sm font-medium leading-6 text-gray-900" for="fecha"
              >Fecha</label
            >
            <div class="">
              <input
          ...
          <div class="flex justify-center align-middle items-center">
            <label class="mx-2 text-sm font-semibold leading-6 text-gray-900" for="fecha"
              >Monto $</label
            >
            <div class="w-fit">
              <input
        ...
        <div>
          ...
          <button
            class="px-6 py-2 ml-2 text-white bg-blue-500 hover:font-semibold hover:bg-blue-700 rounded"
            @click="saveAbono()"
            type="button"
          >
            Guardar
  ...
  methods: {
    ...
    saveAbono() {
      if (this.validarMontoAbono()) {
        ...
        axios
          .post(api_url + '/abono_registrar', params)
          .then(
            (response) => (
