    ...
    <div class="m-8">
      <div class="flex w-full ml-8 mt-4">
        <div>
          ...
          <input
            type="number"
            class="rounded mx-3 mr-3 w-auto max-w-[100px]"
            min="1"
            v-model="limite"
          />
        </div>
        ...
        <ul class="inline-flex -space-x-px text-base h-10 mx-auto">
          <li v-for="page in listaPaginas">
            <button
              ...
              v-bind:class="{
                'text-black font-bold': page.active == true,
                'rounded-l-lg': page == listaPaginas[0],
                'rounded-r-lg': page == listaPaginas[listaPaginas.length - 1],
...
export default {
  ...
  methods: {
    obtenerProductos() {
      axios
        .get(api_url + '/productos/paginacion/5?sort_by=' + this.sort_by)
        .then((response) => {
