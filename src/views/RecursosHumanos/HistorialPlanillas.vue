<template>
  <main>
      ...
      <div class="bg-red-50 border-l-8 border-red-900">
        <div class="flex items-center">
          <div class="p-2">
            <div class="flex items-center">
              <div class="ml-2">
                <svg class="h-8 w-8 text-red-900 mr-2 cursor-pointer" xmlns="http://www.w3.org/2000/svg" fill="none"
    ...
    <div>
      <div class="max-w-4xl mx-auto my-4 text-right">
        <div>
          <label for="opcionFiltro" class="font-bold">Seleccione el año</label>
        </div>
        <select name="" id="" v-model="anioFiltro" class="w-[11%] border-slate-400 rounded"
    ...
    <div class="card max-w-4xl auto m-auto shadow-md">

      <table class="w-full m-auto mt-[2px]">
        <tr class="text-gray-400 bg-gray-50 border-b">
          ...
          <th class="p-2 font-semibold w-4">ACCIONES</th>
        </tr>
        <tbody class="bg-white">
          <tr class="border-b cursor-pointer hover:bg-slate-50" v-for="planilla in controlPagina.datosPagina"
            :key="planilla.id" @click="verDetallePlanilla(planilla.id)">
            <td class="text-center p-[1%]">{{ planilla.id }}</td>
            <td class="text-center p-[1%]">
              {{ obtnerPeriodoFechas(planilla.fecha_inicio, planilla.fecha_fin) }}
            </td>
            <td class="text-center p-[1%]">
              <span class="p-1 rounded text-white text-sm"
                :class="[planilla.date_emision_boleta ? ' bg-emerald-500' : 'bg-orange-400']">
                {{ planilla.date_emision_boleta ? 'Emitida' : 'No emitida' }}
              </span>
            </td>
            <td class="text-center">
              <router-link to="" class="bg-blue-500 hover:bg-indigo-800 text-white p-2 rounded text-center">
                Ver
              </router-link>
            </td>
          </tr>
