      ...
      <Form>
        <div class="container grid lg:grid-cols-6 rounded-md m-auto p-6 pt-4 w-4/5 bg-white shadow">
          <div class="lg:col-span-1">
            <div class="mb-4">
              <label for="fecha_entrega" class="block text-sm font-medium leading-6 text-gray-900"
                >Fecha de entrega</label
              >
              <div class="mt-2">
                <Field
                  @input="actualizarPedidos"
                  v-model="hoja_de_ruta.fecha_entrega"
                  name="fecha_entrega"
                  id="fecha_entrega"
                  type="Date"
            ...
            <div class="mb-4">
              <label for="repartidor" class="block text-sm font-medium leading-6 text-gray-900"
                >Repartidor</label
              >
              <div class="mt-2">
                <select
            ...
            <div class="mb-4">
              <label for="total" class="block text-sm font-medium leading-6 text-gray-900"
                >Total (USD)</label
              >
              <div class="mt-2">
            ...
            <div class="p-4">
              <div class="w-full grid justify-items-center">
                <button
                  @click="showModal"
                  type="button"
                  class="btn bg-green-500 hover:bg-green-600 py-2 px-4 rounded text-white"
                >
                  Agregar Pedidos
                </button>
              </div>
              ...
              <tbody class="text-center">
                <tr v-for="pedido in pedidos_factura" class="border-b">
                  ...
                  <td class="whitespace-nowrap px-6 py-4">
                    <RouterLink
                      :to="'/facturacion/detail_sales/' + pedido.id_venta"
                      target="_blank"
                    ...
                    <button
                      type="button"
                      @click="quitarFacturasDomicilio(pedido.id_venta)"
                      class="bg-transparent hover:bg-red-500 text-red-700 font-semibold hover:text-white py-2 px-4 border border-red-500 hover:border-transparent rounded-full m-1"
                    >
                      Quitar
                    </button>
                  </td>
                ...
                <tr v-for="pedido in pedidos_fiscal" class="border-b">
                  ...
                  <td class="whitespace-nowrap px-6 py-4">
                    <RouterLink
                      :to="'/facturacion/detail_cf/' + pedido.id_creditofiscal"
                      target="_blank"
                      class="bg-transparent hover:bg-blue-500 text-green-700 font-semibold hover:text-white py-2 px-4 border border-green-500 hover:border-transparent rounded-full m-1"
                    >
                      ver</RouterLink
                    ...
                    <button
                      type="button"
                      @click="quitarCreditoFiscalDomicilio(pedido.id_creditofiscal)"
                      class="bg-transparent hover:bg-red-500 text-red-700 font-semibold hover:text-white py-2 px-4 border border-red-500 hover:border-transparent rounded-full m-1"
                    >
                      Quitar
                    </button>
          ...
          <div class="w-min-full flex items-center justify-center col-span-1 lg:col-span-6">
            <button class="bg-red-500 hover:bg-red-600 text-white py-2 px-4 rounded mx-2">
              Cancelar
            </button>
            <button
              @click="saveHojaDeRuta"
              type="button"
              class="bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded mx-2"
            >
              Guardar
            </button>
          </div>
      ...
      >
        <div class="w-11/12 md:w-10/12 mx-auto my-2 rounded-lg bg-slate-50" id="card">
          ...
          <div class="overflow-y-auto max-h-3/12" id="body" style="max-height: 70vh">
            <div
              class="min-w-full p-4 text-center"
              v-if="facturas.length == 0 && creditosFiscales.length == 0"
            >
              <span class="w-full text-center text-slate-500"
                >No se encontraron pedidos sin asignar a Hoja de Ruta para el
              ...
              <tbody class="text-center">
                <tr v-for="pedido in facturas" class="border-b">
                  ...
                  <td class="whitespace-nowrap px-6 py-4">
                    ...
                    <button
                      type="button"
                      @click="addFacturasDomicilio(pedido.id_venta)"
...
export default {
  ...
  methods: {
    getHojaDeRUta() {
      axios
        .get(api_url + '/hoja_de_ruta/' + this.id_hojaDeRuta)
        .then((response) => this.setDatosDeHojaDeRuta(response.data.hoja))
    },
    ...
    setPedidosFiscal(listaPedidosFiscal) {
      listaPedidosFiscal.forEach((pedidoFiscal) => {
        pedidoFiscal.credito_fiscal.id_cliente =
          pedidoFiscal.credito_fiscal.cliente.distintivo_cliente
    ...
    getEmpleados() {
      axios
        ...
        .then((response) => (this.empleados = this.getRepartidores(response.data.data)))
    },
    ...
    getCreditosFiscales() {
      ...
      axios
        .post(api_url + '/creditos_fiscales_domicilio', params)
        .then((response) => (this.creditosFiscales = response.data.creditos))
    },
    getFacturas() {
      let params = {
        fecha: this.hoja_de_ruta.fecha_entrega
      ...
      axios
        .post(api_url + '/facturas_domicilio', params)
    ...
    saveHojaDeRuta() {
      if (this.hoja_de_ruta.id_empleado == null || this.hoja_de_ruta.id_empleado == '') {
        ...
        axios
          .put(api_url + '/hoja_de_ruta/' + this.id_hojaDeRuta, params)
          .then((response) => {
