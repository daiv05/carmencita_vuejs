<template>
      ...
      <div>
        <div class="flex bg-white mx-auto p-5 shadow-md justify-between">
          ...
          <div class="flex items-center rounded-[4.44px] bg-[#637381]">
            <RouterLink
              v-if="hoja_ruta"
              :to="'/facturacion/hoja_de_ruta/update/' + hoja_ruta.id_hr"
              id="show-modal"
        ...
        <div class="tab-content flex-grow">
          <div class="p-4 bg-white">
            <div class="flex pb-36">
              <div class="w-full h-full pt-4 overflow-auto">
                <div class="flex justify-start items-center pb-6">
                  <label class="text-base font-bold"> Fecha de reparto: </label>
                  <input
                    type="date"
                    name="fecha_hoja_ruta"
                    v-model="hoja_ruta.fecha_entrega"
                    disabled
                  ...
                  <input
                    ...
                    :value="
                      hoja_ruta.empleado.primer_nombre + ' ' + hoja_ruta.empleado.primer_apellido
                    "
                  />
                </div>
                <div class="flex justify-center w-full">
                  <table class="table-striped w-full shadow-lg">
                    ...
                    <tbody>
                      <tr
                        v-for="(fila, index) in hoja_ruta.venta_domicilio"
                        :key="fila.id_hr"
                      ...
                      <tr
                        v-for="(fila, index) in hoja_ruta.credito_fiscal_domicilio"
                        :key="fila.id_hr"
                        class="border-b-2 border-black-400 bg-black-300"
                      >
                        <td class="text-center">{{ count + index + 1 }}</td>
                        <td class="text-center">
                          {{ fila.credito_fiscal.cliente.distintivo_cliente }}
                        </td>
                        <td class="text-center">{{ fila.credito_fiscal.fecha_credito }}</td>
                        <td class="text-center">
                          $ {{ Number(fila.credito_fiscal.total_credito).toFixed(2) }}
                        </td>
                        <td class="text-center">Crédito Fiscal</td>
                        <td class="flex justify-end py-2 items-center">
                          <button
                            @click="imprimir_credito_domicilio(fila)"
                            class="text-center bg-emerald-600 hover:bg-emerald-800 md:text-sm text-xs text-white font-medium py-2 px-2 mx-2 rounded"
                          >
                            Imprimir
                          </button>
                          <router-link
                            v-if="fila.esta_cancelado == 0"
                            :to="{
                              name: 'modificar_pedido_credito',
                              params: { id: fila.id_creditofiscal }
                            }"
                            class="text-center bg-indigo-600 hover:bg-indigo-800 md:text-sm text-xs text-white font-medium py-2 px-2 mx-2 rounded"
                          >
                            Editar Pedido
            ...
            <div class="flex pl-8 justify-between">
              ...
              <button
                v-if="!hoja_ruta.esta_entregado"
                @click="marcar_hr_entregada()"
  ...
  <Teleport to="body">
    <ModalConfirmarPagoDomicilio
      :show="showModal_pago_individual"
      :factura="factura_modal"
      :total="total_factura_modal"
      @confirmed="pago_realizado()"
      @close="showModal_pago_individual = false"
  ...
  <Teleport to="body" v-if="hoja_ruta != null">
    <ModalConfirmarEntregasHoja
      @close="showModal_entregas_all = false"
      :show="showModal_entregas_all"
      :id_hoja="hoja_ruta.id_hr"
      @confirmed="confirmacion_entregas()"
    >
    </ModalConfirmarEntregasHoja>
  </Teleport>
</template>
...
export default {
  ...
  methods: {
    obtener_detalles_ruta() {
      axios
        .get(api_url + '/hoja_de_ruta/' + this.ruta)
        .then((res) => {
          ...
          this.hoja_ruta.credito_fiscal_domicilio.forEach((c) => {
            ...
            c.credito_fiscal.fecha_credito = moment(c.credito_fiscal.fecha_credito).format(
              'DD/MM/YYYY'
            )
          })
    ...
    imprimir_resumen_hr() {
      axios
        .get(api_url + '/impresion_hoja_de_ruta/' + this.hoja_ruta.id_hr)
        .then((res) => {
          console.log(res)
    ...
    imprimir_credito_domicilio(fila) {
      axios
        .get(api_url + '/impresion_credito_fiscal/' + fila.id_creditofiscal)
        .then((res) => {
          console.log(res)
          this.watch_toast('success', 'Imprimiendo...')
        })
    ...
    imprimir_venta_domicilio(fila) {
      ...
      axios
        .get(api_url + '/impresion_consumidor_final/' + fila.id_venta)
        .then((res) => {
          console.log(res)
          this.watch_toast('success', 'Imprimiendo...')
        })
