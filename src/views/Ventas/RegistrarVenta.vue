        ...
        <div class="tab-content flex-grow">
          ...
          <div class="p-4 bg-white">
            <div class="flex pb-36">
              <div class="w-3/4 pr-4 h-full min-h-screen pt-4 overflow-auto">
                ...
                <div class="flex justify-start items-center pb-6">
                  <label class="text-base font-bold"> Producto </label>
                  <input
                    @input="listener_buscar_codigo_producto()"
                    ref="codigo_bp"
                    ...
                    v-model="producto_codigo"
                  />
                  <div class="sugerencias-container md:col-span-3">
                    <!-- Campo de entrada -->
                    <input
                      @input="listener_producto_nombre()"
                      @focus="mostrar_sugerencias = true"
                    ...
                    <ul
                      class="sugerencias-lista border border-slate-500"
                ...
                <table class="table-fixed w-full shadow-lg">
                  ...
                  <tbody>
                    <tr
                      v-for="(fila, index) in detalle_ventas_lista"
                      :key="fila.id_venta"
                    ...
                    >
                      ...
                      <td class="text-center">
                        <input
                          @change="verificar_precios_y_ofertas(fila)"
                          class="w-auto h-[25px] text-center"
                          type="number"
                          min="1"
                          max="100"
                      ...
                      <td class="text-center">
                        $
                        {{
                          (fila.subtotal_detalle_venta = Number(
                            fila.producto_detalle.precio_unitario * fila.cantidad_prod_venta
                          ).toFixed(2))
                        }}
                      </td>
                      <td class="flex justify-end pr-4 py-2">
                        <button
                          @click="eliminar_detalle_venta(index + 1)"
              ...
              <div
                class="w-1/4 border-l border-gray-300 pl-2 flex-shrink-0 min-h-[200px] overflow-x-auto"
              >
                  ...
                  <div class="flex pl-2">
                    ...
                    <div>
                      ...
                      <div class="flex md:flex-row flex-col items-center py-2">
                        <div class="flex flex-col">
                          <label
                            for="identificador_cliente_credito"
                      ...
                      <div class="flex md:flex-row flex-col items-center py-2">
                        ...
                        <div class="flex flex-col">
                          <label
                            for="departamento_cliente_credito"
                            class="text-black-800 text-sm font-bold leading-tight tracking-normal mb-2"
                          >
                            Departamento
                          </label>
                          <input
                            id="departamento_cliente_credito"
                            type="text"
                      ...
                      <div class="flex md:flex-row flex-col items-center py-2">
                        ...
                        <div class="flex flex-col">
                          ...
                          <input
                            disabled
                            id="municipio_cliente_credito"
                            type="text"
                            name="municipio_cliente_credito"
                            v-model="municipio_cliente"
                            class="text-slate-600 focus:outline-none focus:border focus:border-emerald-700 bg-white font-normal w-auto h-10 flex items-center pl-3 text-sm border-gray-300 rounded border"
                          />
                        </div>
                      </div>
            ...
            <div class="grid grid-cols-12 pl-8">
                    ...
                    <td class="text-center">
                      <div class="flex items-center">
                        ...
                        >
                          {{
                            Number(
                              Number(subtotal_venta) + Number(venta_info.total_descuentos)
                            ).toFixed(2)
                          }}
                        </label>
                      </div>
                    ...
                    <td class="text-center">
                      <div class="flex items-center">
                        ...
                        <input
                          ...
                          placeholder="0.00"
                          disabled
                          v-model="venta_info.total_descuentos"
                        />
                      </div>
                    </td>
                  </tr>
                    ...
                    <td class="text-center">
                      <div class="flex items-center">
                        <input
                          ...
                          placeholder="0.00"
                          disabled
                          v-model="venta_info.total_venta"
                        />
                      </div>
                    </td>
                  </tr>
              ...
              <div
                class="grid grid-cols-12 col-span-8 justify-center items-center py-4 px-4 w-full mt-10"
              >
                <div class="flex justify-end col-span-6">
                  <button
                    v-if="active_tab === 0"
                    @click="register_new_venta(false)"
                    :class="{
                      'bg-emerald-600 hover:bg-emerald-800': active_tab == 1,
                      'bg-indigo-600 hover:bg-indigo-800': active_tab == 0
                    }"
                ...
                <div class="flex justify-end align-bottom col-span-6">
                  <a
                    @click="restaurar_ultima_factura()"
                    class="text-sm font-light underline"
                    href="#"
                    >Restaurar última venta</a
                  >
                </div>
              </div>
            </div>
  ...
  data() {
    return {
      ...
      detalle_venta: {
        ...
        subtotal_detalle_venta: 0.0,
        descuentos_detalle: 0.0
      },
      //Objeto Venta
      venta_info: {
        id_venta: 0,
  ...
  methods: {
    ...
    listener_cliente_identificador() {
      if (this.campo_identificador_cliente) {
        this.sugerencias_cliente = this.clientes.filter((cliente) => {
          return cliente.distintivo_cliente
            .toLowerCase()
            .includes(this.campo_identificador_cliente.toLowerCase())
        })
      } else {
    ...
    get_producto_segun_nombre() {
      return new Promise((resolve, reject) => {
        ...
        axios
          .get(api_url + '/productos/buscar/' + this.producto_nombre)
          .then((res) => {
            if (res.data.producto.esta_disponible == false) {
              this.watch_toast('error', 'Producto no disponible actualmente')
              return
    ...
    add_detalle_venta() {
      ...
      const producto_ya_agregado = this.detalle_ventas_lista.find((detalle) => {
        return (
          detalle.producto_detalle.codigo_barra_producto === producto_copia.codigo_barra_producto
        )
      })
    ...
    verificar_precios_y_ofertas(detalle) {
      return new Promise((resolve, reject) => {
        ...
        if (detalle.producto_detalle.ofertas_vigentes.length > 0) {
          var ofertasOrdenadas = detalle.producto_detalle.ofertas_vigentes.sort(
            (a, b) => a.cantidad_producto - b.cantidad_producto
          )
        ...
        var preciosOrdenados = detalle.producto_detalle.precio_unidad_de_medida.sort(
          (a, b) => a.cantidad_producto - b.cantidad_producto
        )
        ...
        if (detalle.cantidad_prod_venta < precioUnidadCercano.cantidad_producto) {
          detalle.producto_detalle.precio_unitario =
            detalle.producto_detalle.precio_unitario_original
          resolve()
        } else {
          ...
          detalle.producto_detalle.precio_unitario = (
            parseFloat(precioUnidadCercano.precio_unidad_medida_producto) /
            parseFloat(precioUnidadCercano.cantidad_producto)
          ).toFixed(4)
    ...
    register_venta(detalles_listado_limpio, is_domicilio) {
      ...
      axios
        .post(api_url + '/ventas/registrar/', datos_ventas)
        .then((response) => {
          console.log(response)
          is_domicilio
            ? this.watch_toast('success', 'Pedido a domicilio registrado')
    ...
    register_credito(detalles_listado_limpio, is_domicilio) {
      ...
      axios
        .post(api_url + '/creditos/registrar/', datos_ventas)
        .then((response) => {
          console.log(response)
          is_domicilio
            ? this.watch_toast('success', 'Pedido a domicilio registrado')
    ...
    handle_error(error, operation) {
      this.watch_toast(
        'error',
        error.response?.data.mensaje || `Ocurrió un error al registrar el ${operation}`
      )
      throw error
    },
