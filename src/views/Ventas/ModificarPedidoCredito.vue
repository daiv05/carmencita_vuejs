                  ...
                  <tbody>
                    ...
                    >
                      ...
                      <td class="text-center">
                        <input
                          @change="watch_cantidad_producto(fila)"
                      ...
                      <td class="flex justify-end pr-4 py-2">
                        <button
                          @click="eliminar_detalle_venta(index)"
                    ...
                    <td class="text-center">
                      <div class="flex items-center">
                        ...
                        <input
                          ...
                          placeholder="0.00"
                          disabled
                          v-model="credito_fiscal_info.total_descuentos"
                        />
                      </div>
                    </td>
                  </tr>
                    ...
                    <td class="text-center">
                      <div class="flex items-center">
                        <input
                          ...
                          placeholder="0.00"
                          disabled
                          v-model="credito_fiscal_info.total_credito"
                        />
                      </div>
                    </td>
                  </tr>
...
export default {
  ...
  watch: {
    ...
    detalle_ventas_lista: {
      handler() {
        ...
        this.credito_fiscal_info.total_credito =
          this.credito_fiscal_info.total_credito - this.credito_fiscal_info.total_descuentos
        ...
        this.credito_fiscal_info.total_credito = Number(
          this.credito_fiscal_info.total_credito
        ).toFixed(2)
        this.subtotal_venta = Number(this.subtotal_venta).toFixed(2)
        this.credito_fiscal_info.total_descuentos = Number(
          this.credito_fiscal_info.total_descuentos
        ).toFixed(2)
      },
      deep: true
    }
  ...
  methods: {
    getCreditoFiscal() {
      axios.get(api_url + '/creditos_detalle/' + this.id).then((response) => {
        ;(this.credito_fiscal_info = response.data),
          ...
          response.data.credito_fiscal_domicilio == null
      ...
      return new Promise((resolve, reject) => {
        try {
          ...
          if (element.producto.ofertas_vigentes?.length > 0) {
            var ofertasOrdenadas = element.producto.ofertas_vigentes.sort(
        ...
        if (unidadesMedida) {
          unidadesMedida.forEach((unidadMedida) => {
            if (
              Number(element.cantidad_producto_credito) >= Number(unidadMedida.cantidad_producto)
            ) {
              precio = Number(
                unidadMedida.precio_unidad_medida_producto / unidadMedida.cantidad_producto
              ).toFixed(4)
            }
          })
        }

        element.subtotal_detalle_credito = Number(
          element.cantidad_producto_credito * precio
        ).toFixed(2)
    ...
    add_detalle_venta() {
      ...
      return new Promise((resolve, reject) => {
        ...
        this.producto_nombre = ''
        this.contador_tabla++
        this.calcularSubtotalDetalleVenta(
          this.detalle_ventas_lista[this.detalle_ventas_lista.length - 1]
        )
    ...
    verificar_unidad_medida(detalle) {
      return new Promise((resolve, reject) => {
        ...
        var preciosOrdenados = detalle.producto.precio_unidad_de_medida.sort(
          (a, b) => a.cantidad_producto - b.cantidad_producto
        )
    ...
    register_new_venta() {
      ...
      /*if (this.active_tab == 0) {
        ...
        axios
          .put(
            api_url + '/modificar_pedido_credito/' + this.id,
            (datos_ventas = {
              credito: {
