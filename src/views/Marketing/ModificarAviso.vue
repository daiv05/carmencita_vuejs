<template>
  <main class="bg-slate-50">
    ...
    <div
      class="p-4 mb-4 text-sm text-green-800 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400 z-999 fixed top-20"
      role="alert"
      v-if="mensajeExito != ' '"
    >
    ...
    <div
      ...
      v-if="listaErrores"
    >
      <p v-for="error in listaErrores"><span class="font-medium">Error</span> {{ error }}</p>
    </div>
    <Form @submit="enviarDatos($event)">
      <div
        class="grid grid-cols-10 gap-1 w-[80%] m-auto bg-white p-[2%] rounded-[2px] mt-[1%]"
        ...
        <div class="col-span-3 h-[100%]">
          <div>
            ...
            <input
              type="date"
              id="fecha_creacion"
              class="p-[5px] rounded-[5px] border border-slate-300 w-[100%]"
              disabled
...
export default {
  ...
  methods: {
    ...
    cargarDatos() {
      axios
        .get('/api/avisos/' + this.$route.params.idAviso)
        .then((response) => {
    ...
    enviarDatos(event) {
      ...
      axios
        .put('/api/avisos/' + this.$route.params.idAviso, parametros)
        .then((response) => {
