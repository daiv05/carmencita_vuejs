      ...
      <div>
        <div class="flex bg-white mx-auto p-5 shadow-md justify-between">
          ...
          <div class="flex items-center rounded-[4.44px] bg-[#637381]">
            <router-link
              to="/ventas_inventarios/registrar_credito_proveedor"
          ...
          <div class="col-span-3">
            <div class="mb-4 font-semibold">
              ...
              <div class="flex border border-slate-300 rounded w-10/12 px-2 py-1">
                <CalendarIcon class="m-0 h-5 w-5 font-bold" />
                {{ formatFecha(credito.fecha_credito) }}
              </div>
            </div>
            <div class="mb-4 font-semibold">
              ...
              <div class="flex border border-slate-300 rounded w-10/12 px-2 py-1">
                <CalendarIcon class="m-0 h-5 w-5 font-bold" />
                {{ formatFecha(credito.fecha_limite_pago) }}
              </div>
            </div>
            <div class="mb-4 font-semibold">
              Credito inicial:
              ...
              <div class="m-0 shadow p-2 rounded">
                <table v-if="credito.abonos.length > 0" class="table w-full rounded">
                  ...
                  <tbody class="text-center">
                    <tr
                      v-for="(abono, index) in credito.abonos"
                    ...
                    <tr>
                      <td class="px-2 py-4 font-semibold" colspan="3">
                        Total abonado: $ {{ Number(getTotalAbonado()).toFixed(2) }}
                      </td>
                    </tr>
                  </tbody>
                </table>
                <span v-if="credito.abonos.length == 0" class="text-md text-slate-400"
                  >No se han registrado abonos para este crédito</span
                >
              </div>
            </div>
          </div>
