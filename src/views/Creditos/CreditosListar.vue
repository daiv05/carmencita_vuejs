    ...
    <div class="bg-slate-100 pb-4">
      <div>
        <div class="flex bg-white mx-auto p-5 shadow-md justify-between">
          ...
          <div class="flex items-center rounded-[4.44px] bg-[#637381]">
            <router-link
              to="/registrar_credito_proveedor"
      ...
      <div class="grid grid-cols-3 p-6 pt-4 w-4/5 mx-auto">
        <div class="col-span-1 flex flex-col justify-center p-2">
          <label class="block text-sm font-medium leading-6 text-gray-900" for="estado"
            >Estado</label
          >
          <div class="">
            <select
        ...
        <div class="col-span-1 flex flex-col justify-center p-2">
          <label class="block text-sm font-medium leading-6 text-gray-900" for="proveedor"
            >Proveedor</label
          >
          <div>
            <select
        ...
        <div class="col-span-1 flex justify-center align-middle">
          <button
            @click="getCreditos()"
            class="bg-blue-500 px-3 py-2 rounded text-slate-50 h-fit flex m-auto"
          >
            Aplicar filtros
          </button>
        </div>
        ...
        <table class="table w-full max-h-screen rounded-md">
          <thead class="border-b bg-slate-100">
            <tr class="text-center">
              ...
              <td scope="col" class="px-6 py-4 text-xs text-gray-500 font-semibold">
                FECHA LIMITE
              </td>
          ...
          <tbody class="text-center">
            <tr
              ...
              class="border-b hover:bg-slate-100 hover:shadow"
            >
              ...
              <td class="whitespace-nowrap px-4 py-4">
                {{ moment(credito.fecha_limite_pago).format('DD/MM/yyyy') }}
              </td>
              <td v-if="credito.pendiente > 0" class="whitespace-nowrap px-4 py-4">
      ...
      <nav aria-label="Page navigation example" class="flex py-4 w-full">
        <ul class="inline-flex -space-x-px text-base h-10 mx-auto">
          <li v-for="page in paginas">
            <button
              type="button"
              @click="linksPagination(page.url)"
              v-bind:class="{
                'bg-blue-600 text-white': page.active == true,
                'rounded-l-lg': page == paginas[0],
                'rounded-r-lg': page == paginas[paginas.length - 1],
                'cursor-not-allowed': !page.url
              }"
...
export default {
  ...
  methods: {
    getCreditos() {
      ...
      axios
        .post(api_url + '/lista_creditos_proveedores', params)
        .then(
          (response) => (
